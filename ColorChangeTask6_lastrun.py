...
expName = 'ColorChangeTask'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/maxkaplan/Documents/ColorChangeTask6/ColorChangeTask6_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    s1 = visual.ShapeStim(
        ...
        ori=0.0, pos=(-0.18, 0.31), draggable=False, anchor='center',
        lineWidth=1.0,
    ...
    s3 = visual.ShapeStim(
        win=win, name='s3',
        size=[0.08, 0.08], vertices='circle',
        ori=0.0, pos=(0.36, 0.00), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    for thisTrial in trials:
        ...
        s5.setFillColor(col5)
        s5.setLineColor(col5)
        s6.setFillColor(col6)
        s6.setLineColor(col6)
        ...
        t5.setFillColor(tcol5)
        t5.setLineColor(tcol5)
        t6.setFillColor(tcol6)
        t6.setLineColor(tcol6)
        ...
        while continueRoutine:
            ...
            # if t6 is starting this frame...
            if t6.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                t6.frameNStart = frameN  # exact frame index
                t6.tStart = t  # local t and not account for scr refresh
                t6.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(t6, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 't6.started')
                # update status
                t6.status = STARTED
                t6.setAutoDraw(True)
